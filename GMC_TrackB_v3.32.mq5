//+------------------------------------------------------------------+
//|                  G MONEY CORE — TRACK B v3.32                    |
//|  Min Lot = 0.01 | MT5 | cleaned & bug-fixed build                |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
double        pip;
int           handle_ema_trend, handle_ema_signal, handle_atr;
...
// Select ONLY this EA's position on this symbol (safe on hedging accounts)
...
// LOT CALCULATION — enforces 0.01 minimum and Max cap
double CalculateLotSize(double sl_pips, bool &valid)
{
   ...
   if(ideal_lot < Min_Lot_Size)
   {
      Log("Calculated lot " + DoubleToString(ideal_lot,2) + " < min, using: " + DoubleToString(Min_Lot_Size,2));
      ideal_lot = Min_Lot_Size;
   }
   if(ideal_lot > Max_Lot_Size)
   {
      Log("Calculated lot " + DoubleToString(ideal_lot,2) + " > max, capping: " + DoubleToString(Max_Lot_Size,2));
      ideal_lot = Max_Lot_Size;
   }
...
bool SafetyChecks()
{
   ...
   if((daily_start_balance - current) >= daily_max_loss)
   {
      Log("Daily loss limit reached — trading paused");
      return false;
   }
   if(consecutive_losses >= Max_Consec_Losses)
   {
      ...
      cooldown_until = 0;
      Log("Cooldown finished — trading resumed");
   }
   return true;
}
...
int OnInit()
{
   ...
   if(handle_ema_trend == INVALID_HANDLE || handle_ema_signal == INVALID_HANDLE || handle_atr == INVALID_HANDLE)
   {
   ...
   trade.SetTypeFillingBySymbol(_Symbol);   // auto-pick a filling mode the broker supports
   trade.SetMarginMode();

   daily_start_balance = AccountInfoDouble(ACCOUNT_BALANCE);
   daily_max_loss      = daily_start_balance * (Daily_Max_Loss_Pct / 100.0);
   Log("GMC v3.32 initialized | Min Lot = " + DoubleToString(Min_Lot_Size,2) + " | Ready");
...
void OnTick()
{
   ...
   double ema1[], ema2[], atr[], o[], h[], l[], c[];
   ArraySetAsSeries(ema1, true); ArraySetAsSeries(ema2, true); ArraySetAsSeries(atr, true);
   ...
   {
      Log("Spread too high: " + DoubleToString(spread,1) + "p — waiting");
      return;
   }
   ...
   if(long_ok && c[1] > h[2])
   {
      ...
      if(trade.Buy(lot, _Symbol, entry, sl, tp, "GMC LONG"))
         Log("LONG opened | Lot: " + DoubleToString(lot,2) + " | SL: " + DoubleToString(sl_pips,0) + "p");
   }
   else if(short_ok && c[1] < l[2])
   {
      ...
      if(trade.Sell(lot, _Symbol, entry, sl, tp, "GMC SHORT"))
         Log("SHORT opened | Lot: " + DoubleToString(lot,2) + " | SL: " + DoubleToString(sl_pips,0) + "p");
   }
}

//+------------------------------------------------------------------+
// POSITION MANAGEMENT (per-position state, stable original risk)
//+------------------------------------------------------------------+
void ManagePosition()
{
   ...
   // New position -> reset state and capture ORIGINAL risk from the initial SL
   if(ticket != g_ticket)
   {
      g_ticket    = ticket;
      g_be        = false;
   ...
   if(!g_be && profit >= risk * BE_Trigger_R)
   {
      ...
      if(new_sl != sl && trade.PositionModify(ticket, new_sl, tp))
      {
         g_be = true; sl = new_sl;
         Log("Break-even activated");
      }
   }

   // Partial close ONCE at TP1
   if(!g_partial && profit >= risk * TP1_ATR_Multiplier)
   {
      ...
      if(amt >= Min_Lot_Size && amt < vol)
      {
         if(trade.PositionClosePartial(ticket, amt)) { g_partial = true; Log("Partial close at TP1"); }
      }
      else g_partial = true; // too small to split — latch so we don't retry every tick
   }
...
void OnTrade()
{
   if(SelectMyPosition()) return;  // still in a trade
   ...
   for(int i = HistoryDealsTotal()-1; i >= 0; i--)
   {
      ...
      if(t == last_counted) break;   // already processed this close
      last_counted = t;
