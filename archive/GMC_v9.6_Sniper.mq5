//+------------------------------------------------------------------+
//|  GMC_v9.6_Sniper.mq5  — v9.5 + ADX trend-strength regime gate    |
...
int hMA, hRSI, hATR, hH4MA, hH1MA, hADX;
double pip;
datetime lastBar = 0;
...
int OnInit()
{
   ...
   if(hMA==INVALID_HANDLE||hRSI==INVALID_HANDLE||hATR==INVALID_HANDLE||hH4MA==INVALID_HANDLE||hH1MA==INVALID_HANDLE||hADX==INVALID_HANDLE)
      return(INIT_FAILED);
...
void OnDeinit(const int r){ IndicatorRelease(hMA);IndicatorRelease(hRSI);IndicatorRelease(hATR);IndicatorRelease(hH4MA);IndicatorRelease(hH1MA);IndicatorRelease(hADX); }
...
void OnTick()
{
   ...
   double adx=Bf(hADX,1);
   bool trendStrong = !UseADX || (adx!=EMPTY_VALUE && adx>=ADXMin);
   bool sess=InSession();
   bool coolOk = (barIdx-lastEntryBar)>=CooldownBars;
   bool cascCoolOk = (barIdx-lastCascBar)>=CascCooldown;

   bool buySig  = bullPerfect && bullConf && volaOk && sess && coolOk && trendStrong;
   bool sellSig = bearPerfect && bearConf && volaOk && sess && coolOk && trendStrong;
   ...
   bool cascSell = UseCascade && bear>=CascMinScore && moveDn>=CascPip1 && cVol && h4bear && h1bear && rsi1<50 && sess && cascCoolOk && c1<ma1 && trendStrong && (moveDn>=CascPip3 || cSizeOk);
   bool cascBuy  = UseCascade && bull>=CascMinScore && moveUp>=CascPip1 && cVol && h4bull && h1bull && rsi1>50 && sess && cascCoolOk && c1>ma1 && trendStrong && (moveUp>=CascPip3 || cSizeOk);
